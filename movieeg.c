...
// Function to generate a unique random 3-digit ID
int generateUniqueID(struct Student *head) {
    int id;
    bool isUnique;
    ...
    do {
        ...
        while (current != NULL) {
            if (current->id == id) {
                isUnique = false; // ID already exists
                break;
            }
...
struct Student* addStudent(struct Student *head) {
    int uniqueID = generateUniqueID(head); // Generate a unique random 3-digit ID
...
int main() {
    ...
    srand(12345); // You can change the seed for different sequences

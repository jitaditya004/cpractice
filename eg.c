...
struct Student* addStudent(struct Student *head, int id) {
    struct Student *newStudent = createStudent(id);
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            case 1:
                printf("Enter unique ID for new student: ");
                scanf("%d", &id);
                getchar(); // Consume newline character
                head = addStudent(head, id);
                break;
